...
gz_fish=ggplot(data=eco.dat%>%filter(Fish!=0))+
  ...
  labs(y="TCT", x="Sample Replicate")+theme_bw()+ scale_y_continuous(breaks=c(0,12.5,25),
                                                                     labels=c("0", "12.5", "25"),limits=c(0,25))+ggtitle("Plot of TCT by Sample replicate by Fish and Tank.")+ theme(strip.background =element_rect(fill="lightblue"))
...
gz_morezeros=ggplot(data=eco.dat%>%filter(Fish==0)%>%filter(Sort.Code %in% 141:161))+
  ...
  ggtitle("Plot of TCT for Zero Fish (Background Signal).")+ 
...
dkk=data.frame(itank,round(kt.intercept,3),round(kt.slope,3),kt.r2)
names(dkk)=c("Tank","Intercept","Slope","$R^{2}$")

kzz=kable(dkk,caption="Summary of Tank Specific Simple Linear Regressions",format="latex",booktabs=T,escape = FALSE)%>%
  kable_styling(latex_options = 'striped')
...
gg_simple_median=ggplot(data=gframe)+
  ...
  theme(panel.grid.minor = element_blank(),
        panel.background = element_blank(),axis.line = element_line(colour = "black"))+labs(title="Median Transformed CT by Log2 Fish Biomass (gram)")+ theme(
...
gg_par_median=gg_par_median+labs(title="Median Transformed CT by Log2 Fish Biomass (gram) \nwith differing intercepts")+ theme(
  legend.position = c(1, 0.05),
  legend.justification = c("right", "bottom"))
...
gg_tank_linear=ggplot(data=gframe)+
  ...
  ylab("Median Transformed CT")+ 
  theme(panel.grid.minor = element_blank(),
        panel.background = element_blank(),axis.line = element_line(colour = "black"))+scale_y_continuous(breaks=10:25)+labs(title="Median Transformed CT by Log2 Fish Biomass (gram) \nwith Tank Specific Linear fits")+ theme(legend.position = c(1, 0.05),legend.justification = c("right", "bottom"))
...
gn=ggplot(data=new.tank)+
  ...
  ggtitle("Line of best fit for Log2(biomass) vs Mean TCT \ncollapsed over each tank")
...
gn2=ggplot(data=new.tankmed)+
  ...
  scale_x_continuous(position = "bottom") +
  scale_x_continuous(sec.axis=sec_axis(trans=~ . * 1, name=TeX("Coho biomass ($log_{2}$ g)")),labels=iFish,breaks=label.pos,position='top')+
  ggtitle("Line of best fit for Log2(biomass) vs Median TCT \ncollapsed over each tank")
...
dkk2=data.frame(itank,round(kt2.intercept,3),round(kt2.slope,3),kt2.r2)
names(dkk2)=c("Tank","Intercept","Slope","$R^{2}$")

kzz2=kable(dkk2,caption="Summary of Tank Specific Simple Linear Regressions",format="latex",booktabs=T,escape = FALSE)%>%
  kable_styling(latex_options = 'striped')
